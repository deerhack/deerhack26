...
const Navbar = () => {
  ...
  const btnContainer = "relative flex items-center justify-center w-[56px] h-[56px] transition-transform active:scale-95";
        ...
        {!isOpen && (
          ...
          >
            <div className={btnContainer}>
              ...
              <Image src={hamburgerIcon} alt="Open" width={24} height={24} className="relative z-10" />
            </div>
          </div>
        )}
          ...
          >
            <div className={btnContainer}>
              <Image 
                ...
                className="object-contain pointer-events-none" 
              />
              <Image src={hamburgerIcon} alt="Close" width={24} height={24} className="relative z-10" />
            </div>
          </div>

          {/* Sidebar Content */}
          <div className={`flex flex-col h-full w-full p-8 md:p-12 pt-12 transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0"}`}>
            <div className="flex items-center gap-4 mb-12">
                 <Image src={DeerhackLogo} alt="Logo" width={50} height={50} className="md:w-[60px] md:h-[60px]" />
            ...
            <div className="mt-10 flex flex-col gap-4">
              <div className="relative flex items-center mb-1">
                <span className="text-[11px] text-gray-400 pr-3 uppercase tracking-widest font-bold">Resources</span>
            ...
            <div className="mt-auto pb-10 flex justify-center items-center gap-[20px] border-t border-gray-800/40 pt-8">
              <SocialLink href="https://www.facebook.com/people/DeerHack-2023/100089906884851/" src={fbIcon} alt="Facebook" />
              <SocialLink href="https://www.instagram.com/deerhack2026/" src={instaIcon} alt="Instagram" />
              <SocialLink href="https://www.linkedin.com/company/deerhack-dwit/" src={linkedinIcon} alt="LinkedIn" />
            </div>
          </div>
        </aside>

        {isOpen && <div className="fixed inset-0 bg-black/30 backdrop-blur-[3px] z-40" onClick={() => setIsOpen(false)} />}
      </div>

      <div className="hidden lg:flex fixed top-8 left-1/2 -translate-x-1/2 z-50 items-center justify-between w-[95%] max-w-7xl">
        <Link href="/" className="hover:scale-105 transition-transform bg-[#110C24CC] p-2 rounded-2xl border border-white/10 backdrop-blur-md">
          <Image src={DeerhackLogo} alt="Logo" width={45} height={45} />
        </Link>

        <div className={`flex items-center gap-8 bg-[#110C24CC] backdrop-blur-md border border-[#646265] px-12 py-4 rounded-full shadow-2xl ${cabinetMedium.className}`}>
...
const NavLink = ({ href, children }: { href: string, children: React.ReactNode }) => (
    <Link href={href} className="text-white/70 hover:text-white transition-colors text-sm font-medium tracking-wide uppercase">
        {children}
    </Link>
)

const SidebarButton = ({ href, label, icon }: { href: string, label: string, icon: any }) => (
  <Link href={href} className="flex items-center gap-5 px-6 py-5 rounded-2xl bg-[#1A1438] hover:bg-[#251C4D] border border-white/5 transition-all text-white/90 group">
...
const SocialLink = ({ href, src, alt }: { href: string, src: any, alt: string }) => (
  <a href={href} target="_blank" rel="noopener noreferrer" className="transition-all hover:scale-125 opacity-70 hover:opacity-100">
