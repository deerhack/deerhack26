...
const AboutCard = function () {
	...
	return (
		<div
			ref={container}
			className="about-card-container h-max w-full md:w-[50rem] xl:w-[76.688rem] md:h-150.5 xl:h-[37.625rem] primary-aboutpage-background rounded-[20px] p-[4px] tems-center relative flex justify-center mx-4"
		>
			<div className="w-full h-full bg-[#130a2c] rounded-2xl p-10 2xl:p-15 flex flex-col justify-center items-start relative">
				{details.map((item, index) => (
					<div
						key={item.id}
						className={`detail-content detail-${index} 2xl:pr-20`}
					>
						<div className="bg-dark-purple rounded-full w-12 lg:w-20 h-12 mb-5 lg:h-20 flex items-center justify-center p-0">
							{item.svg && typeof item.svg !== "string" ? <item.svg /> : null}
						</div>
						<div
							className={`flex  justify-start items-center text-3xl lg:text-6xl text-main-title w-fit  ${cabinetExtraBold.className} text-start`}
						>
							<p className="">{item.title}</p>
						</div>
						<div
							className={`text-magnolia text-base text-justify lg:text-[28px] ${satoshiRegular.className} leading-7 tracking-wide mt-10 pr-5`}
						>
							{item.desc}
						</div>
					</div>
